...
function MedianFilter()
{
    ...
    this.convertImage = async function (imageData, w, h)
    {
        ...
        */
        ...
        if (false)
        { // if (true) : you'll get an equalized image:
        ...
        for (var y = 0; y < h; y++)
        {
            ...
            //Progress bar (first 50%)
            if(Math.round(y / h * 50) % 5 == 0)
            {
                progressBar.style.width = Math.round(y / h * 50) + '%';
                await sleep(0);
            }
        }
...
async function oldMedianFilter(imageData, radius, foundColors, ctx)
{
    ...
    for (var x = 0; x < width; x++)
    {
        ...
        //Progress bar (first 50%)
        if(Math.round(x / width * 50) % 5 == 0)
        {
            progressBar.style.width = Math.round(x / width * 50) + '%';
            await sleep(0);
        }
    }
...
async function medianFilter(imageData, radius, foundColors, ctx)
{
    ...
    medianFilter.maskHeight = radius + 1;
    medianFilter.maskWidth = radius + 1;

    switch(document.getElementById('medianSelect').value)
    {
        case 'fast':
            medianFilter.mode = medianFilter.FAST;
            break
        case 'quality':
            medianFilter.mode = medianFilter.QUALITY;
            break
        case 'old':
            var outputOldMedian = await oldMedianFilter(imageData, radius, foundColors, ctx);

            console.log('oldMedianFilter: ' + (performance.now() - t0));

            return outputOldMedian;
        default:
            medianFilter.mode = medianFilter.FAST;
    }
    
    switch(document.getElementById('shapeSelect').value)
    {
        case 'CIRCULAR':
            medianFilter.shape = medianFilter.CIRCULAR;
            break
        case 'DIAMOND':
            medianFilter.shape = medianFilter.DIAMOND;
            break
        case 'RECTANGULAR':
            medianFilter.shape = medianFilter.RECTANGULAR;
            break;
        default:
            medianFilter.shape = medianFilter.CIRCULAR;
    }

    var output = await medianFilter.convertImage(imageData, imageData.width, imageData.height);
